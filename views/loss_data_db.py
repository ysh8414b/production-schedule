...
def insert_production_status(upload_data, groups_with_items):
    ...
    # 0. 같은 날짜의 기존 업로드 삭제 (중복 방지)
    existing = client.table("production_status_uploads").select("id").eq("upload_date", upload_data["upload_date"]).execute()
    if existing.data:
        for row in existing.data:
            client.table("production_status_uploads").delete().eq("id", row["id"]).execute()
...
def delete_production_status_group(group_id, upload_id):
    """개별 그룹(제품) 삭제 후 업로드 집계 갱신. 남은 그룹이 없으면 업로드도 삭제."""
    client = get_supabase_client()

    # 1. 그룹 삭제 (CASCADE로 items 자동 삭제)
    client.table("production_status_groups").delete().eq("id", group_id).execute()

    # 2. 남은 그룹 확인
    remaining = client.table("production_status_groups").select("id,total_input_kg,total_output_kg,loss_kg").eq("upload_id", upload_id).execute()

    if not remaining.data:
        # 남은 그룹이 없으면 업로드 자체 삭제
        client.table("production_status_uploads").delete().eq("id", upload_id).execute()
    else:
        # 업로드 집계 갱신
        total_input = sum(float(r.get("total_input_kg", 0) or 0) for r in remaining.data)
        total_output = sum(float(r.get("total_output_kg", 0) or 0) for r in remaining.data)
        total_loss = sum(float(r.get("loss_kg", 0) or 0) for r in remaining.data)
        client.table("production_status_uploads").update({
            "total_groups": len(remaining.data),
