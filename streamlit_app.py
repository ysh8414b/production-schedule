...
def create_schedule(sales_file, start_date):
    ...
    # 긴급도 계산
    for plan in production_plan:
        plan['urgency'] = get_urgency(plan['reason'], plan['product'], 0, plan['next_week'])
    
    production_plan.sort(key=lambda x: -x['urgency'])
    ...
    for plan in production_plan:
        ...
        is_next_week = plan.get('next_week', False)
        ...
        for day_idx in valid_days:
            day = DAYS[day_idx]
            current_urgency = get_urgency(reason, p, deadline - day_idx, is_next_week)
...
if menu == "📅 새 스케줄 생성":
    ...
    if uploaded_file and st.button("🚀 스케줄 생성", type="primary"):
