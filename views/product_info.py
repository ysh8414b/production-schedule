...
def _build_inventory_image(product_df, meat_df, base_date_str):
    ...
    for idx, (_, row) in enumerate(meat_df.iterrows()):
        ...
        ax.text(m_code_x, y + row_h / 2, str(row["meat_code"]), fontsize=fs_data, va="center", color="#555")
        ax.text(m_name_x, y + row_h / 2, str(row["meat_name"]), fontsize=fs_data, va="center", color="#000")
        ax.text(m_origin_x, y + row_h / 2, str(row["origin"]), fontsize=fs_data, va="center", color="#555")
        ax.text(m_kg_x, y + row_h / 2, f"{kg:,.1f}" if kg else "0", fontsize=fs_data, va="center", ha="right")
        ax.text(m_box_x, y + row_h / 2, f"{bx:,}", fontsize=fs_data, va="center", ha="right")
...
with tab4:
        ...
        with st.expander("📤 재고 엑셀 업로드", expanded=saved_products.empty and saved_meats.empty):
            ...
            with upload_col_l:
                ...
                if up_products.empty:
                    ...
                    uploaded_prod_inv = st.file_uploader(
                        ...
                        help="'제품 재고' 시트가 포함된 엑셀 파일",
                    )
                    if uploaded_prod_inv:
                        try:
                            xls_p = pd.ExcelFile(uploaded_prod_inv)
                            if "제품 재고" not in xls_p.sheet_names:
                                st.warning("'제품 재고' 시트를 찾을 수 없습니다.")
                            else:
                                p_detail, p_date = _parse_inventory_sheet(xls_p, "제품 재고")
                                # 엑셀에서 상품코드별 잔량 합산
            ...
            with upload_col_r:
                ...
                uploaded_meat_inv = st.file_uploader(
                    ...
                    help="'원육 재고' 시트가 포함된 엑셀 파일",
                )
                if uploaded_meat_inv:
                    try:
                        xls_m = pd.ExcelFile(uploaded_meat_inv)
                        if "원육 재고" not in xls_m.sheet_names:
                            st.warning("'원육 재고' 시트를 찾을 수 없습니다.")
                        else:
                            m_detail, m_date = _parse_inventory_sheet(xls_m, "원육 재고")
    ...
    if saved_products.empty and saved_meats.empty:
        ...
        if inv_sub == "📊 재고 보고서":
            ...
            inv_fig = _build_inventory_image(saved_products, filtered_meats, inv_base_date)
            st.pyplot(inv_fig)
