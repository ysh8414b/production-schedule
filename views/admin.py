...
from utils.auth import is_authenticated, is_admin, get_admin_client, TAB_KEYS, get_anonymous_permissions, save_anonymous_permissions
...
st.divider()
st.subheader("비로그인 사용자 권한")
st.caption("로그인하지 않은 일반 방문자의 탭별 접근 권한을 설정합니다. 기본값은 '조회만'입니다.")
...
with st.form("anon_permissions_form"):
    ...
    if anon_save:
        try:
            save_anonymous_permissions(anon_new_perms)
            st.success("비로그인 사용자 권한 저장 완료!")
            st.rerun()
        except Exception as e:
