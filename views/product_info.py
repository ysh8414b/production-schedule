...
def update_uploaded_product_stocks_bulk(updates):
    """여러 제품 재고 일괄 업데이트.
    updates: list of dict — product_code, current_stock 필수, product_name 선택
    product_name이 포함되면 upsert 1회, 없으면 개별 update.
    """
    if not updates:
        return
    client = get_supabase_client()
    chunk_size = 500
    has_name = all("product_name" in item for item in updates)
    ...
    for i in range(0, len(updates), chunk_size):
        ...
        if has_name:
            ...
            client.table("uploaded_products").upsert(
                upsert_rows, on_conflict="product_code"
            ).execute()
        else:
            for item in chunk:
...
with tab4:
                            ...
                            if "재고집계" not in xls_p.sheet_names:
                                ...
                                if st.button("💾 제품 재고 저장", type="primary", key="inv_prod_save_btn", use_container_width=True):
                                    ...
                                    try:
                                        ...
                                        stock_updates = [{"product_code": r["product_code"], "product_name": r["product_name"], "current_stock": r["remaining_box"]} for r in rows]
    ...
    if saved_products.empty and saved_meats.empty:
        ...
        if inv_sub == "📊 재고 보고서":
            ...
            inv_buf = BytesIO()
            inv_fig.savefig(inv_buf, format="png", dpi=300, bbox_inches="tight",
                            facecolor="white", edgecolor="none")
            inv_buf.seek(0)
            plt.close(inv_fig)

            st.image(inv_buf.getvalue(), use_container_width=True)
