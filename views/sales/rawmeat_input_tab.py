import streamlit as st
import pandas as pd
from views.sales import (
    load_raw_meat_inputs,
    insert_raw_meat_inputs,
    delete_raw_meat_input,
    cleanup_old_raw_meat_inputs,
    load_loss_assignments,
)


def render_rawmeat_input_tab():
    """투입 원육 데이터 관리 탭"""

    # 2개월 이전 데이터 자동 삭제
    cleanup_old_raw_meat_inputs()

    menu = st.radio("선택", [
        "📋 데이터 조회",
        "📤 엑셀 업로드",
        "🗑️ 데이터 삭제"
    ], horizontal=True, key="rawmeat_input_menu")

    st.divider()

    # 업로드 성공 메시지
    if st.session_state.get("_rawmeat_upload_success"):
        st.success(st.session_state["_rawmeat_upload_success"])
        del st.session_state["_rawmeat_upload_success"]

    # ========================
    # 데이터 조회
    # ========================
    if menu == "📋 데이터 조회":
        st.header("투입 원육 현황")

        df = load_raw_meat_inputs()

        if df.empty:
            st.info("투입된 원육이 없습니다. '엑셀 업로드'에서 먼저 업로드해주세요.")
        else:
            # 로스 할당 상태 확인 (loss_assignments 테이블 기반)
            loss_df = load_loss_assignments()
            imported_ids = set()
            if not loss_df.empty and "raw_meat_input_id" in loss_df.columns:
                imported_ids = set(loss_df["raw_meat_input_id"].dropna().astype(int).tolist())

            imported_count = len([rid for rid in df["id"] if rid in imported_ids])
            not_imported_count = len(df) - imported_count

            # 메트릭
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("전체 건수", f"{len(df)}건")
            with col2:
                st.metric("로스 미불러옴", f"{not_imported_count}건")
            with col3:
                st.metric("로스 불러옴", f"{imported_count}건")
            with col4:
                total_kg = df["kg"].fillna(0).astype(float).sum()
                st.metric("총 투입량", f"{total_kg:,.1f}kg")

            st.divider()

            # 날짜 필터
            all_dates = sorted(df["move_date"].dropna().unique().tolist(), reverse=True)
            sel_dates = st.multiselect(
                "📅 날짜 필터",
                options=all_dates,
                default=[all_dates[0]] if all_dates else [],
                placeholder="전체 날짜",
                key="rawmeat_view_dates"
            )

            filtered = df.copy()
            if sel_dates:
                filtered = filtered[filtered["move_date"].isin(sel_dates)]

            if filtered.empty:
                st.info("해당 조건에 데이터가 없습니다.")
            else:
                # 로스 할당 상태 표시
                display_data = []
                for _, row in filtered.iterrows():
                    move_amount = row.get("move_amount", 0)
                    if pd.isna(move_amount):
                        move_amount = 0
                    is_imported = row["id"] in imported_ids
                    display_data.append({
                        "이동일자": row.get("move_date", ""),
                        "상품코드": row.get("meat_code", ""),
                        "상품명": row.get("meat_name", ""),
                        "원산지(등급)": row.get("origin_grade", ""),
                        "Kg": float(row.get("kg", 0) or 0),
                        "이동금액": float(move_amount),
                        "이력번호": row.get("tracking_number", ""),
                        "로스상태": "📉 불러옴" if is_imported else "-",
                    })

                display_df = pd.DataFrame(display_data)
                st.dataframe(
                    display_df.style.format({
                        "Kg": "{:,.1f}",
                        "이동금액": "{:,.0f}",
                    }),
                    use_container_width=True, hide_index=True
                )

    # ========================
    # 엑셀 업로드
    # ========================
    elif menu == "📤 엑셀 업로드":
        st.header("투입 원육 업로드")
        st.caption("엑셀/CSV 파일로 투입된 원육을 업로드합니다.")
        st.markdown("""
        **업로드 양식 (컬럼명)**
        | 이동일자 | 상품코드 | 상품명 | 원산지(등급) | Kg | 이동금액 | 이력번호 |
        |---------|---------|-------|------------|-----|---------|---------|
        | 2025-01-01 | M001 | 소목심 | 호주산(1등급) | 150.5 | 500000 | T20250101-001 |
        """)

        uploaded_file = st.file_uploader(
            "엑셀 또는 CSV 파일 업로드",
            type=["xlsx", "xls", "csv"],
            key="rawmeat_input_upload"
        )

        if uploaded_file:
            try:
                if uploaded_file.name.endswith(".csv"):
                    df_upload = pd.read_csv(uploaded_file)
                else:
                    df_upload = pd.read_excel(uploaded_file)

                # 컬럼 매핑 (유연하게)
                col_map = {}
                for col in df_upload.columns:
                    col_clean = str(col).strip().replace(" ", "")
                    if "이동일자" in col_clean or "일자" in col_clean or "날짜" in col_clean or "date" in col_clean.lower():
                        col_map[col] = "move_date"
                    elif "상품코드" in col_clean or "원육코드" in col_clean or "코드" in col_clean or "code" in col_clean.lower():
                        col_map[col] = "meat_code"
                    elif "상품명" in col_clean or "원육명" in col_clean or "원육" in col_clean or "상품" in col_clean:
                        if "코드" not in col_clean and "code" not in col_clean.lower():
                            col_map[col] = "meat_name"
                    elif "원산지" in col_clean or "등급" in col_clean or "origin" in col_clean.lower():
                        col_map[col] = "origin_grade"
                    elif col_clean.lower() == "kg" or "무게" in col_clean or "중량" in col_clean:
                        col_map[col] = "kg"
                    elif "이동금액" in col_clean or "금액" in col_clean or "amount" in col_clean.lower():
                        col_map[col] = "move_amount"
                    elif "이력번호" in col_clean or "이력" in col_clean or "tracking" in col_clean.lower():
                        col_map[col] = "tracking_number"

                if col_map:
                    df_upload = df_upload.rename(columns=col_map)

                # 필수 컬럼 확인
                required = ["move_date", "meat_name", "kg"]
                missing = [c for c in required if c not in df_upload.columns]
                if missing:
                    st.error(f"필수 컬럼이 누락되었습니다: {', '.join(missing)}")
                    st.info("컬럼명을 확인해주세요: 이동일자, 상품코드, 상품명, 원산지(등급), Kg, 이동금액, 이력번호")
                else:
                    # 선택 컬럼 기본값
                    for col in ["meat_code", "origin_grade", "tracking_number"]:
                        if col not in df_upload.columns:
                            df_upload[col] = ""
                    if "move_amount" not in df_upload.columns:
                        df_upload["move_amount"] = 0

                    # 데이터 정리
                    df_upload["move_date"] = pd.to_datetime(df_upload["move_date"], errors="coerce").dt.strftime("%Y-%m-%d")
                    df_upload["kg"] = pd.to_numeric(df_upload["kg"], errors="coerce").fillna(0)
                    df_upload["move_amount"] = pd.to_numeric(df_upload["move_amount"], errors="coerce").fillna(0)
                    df_upload["meat_code"] = df_upload["meat_code"].fillna("").astype(str).str.strip()
                    df_upload["meat_name"] = df_upload["meat_name"].fillna("").astype(str).str.strip()
                    df_upload["origin_grade"] = df_upload["origin_grade"].fillna("").astype(str).str.strip()
                    df_upload["tracking_number"] = df_upload["tracking_number"].fillna("").astype(str).str.strip()

                    # 유효한 행만
                    valid = df_upload[
                        (df_upload["move_date"].notna()) &
                        (df_upload["meat_name"] != "") &
                        (df_upload["kg"] > 0)
                    ].copy()

                    if valid.empty:
                        st.warning("유효한 데이터가 없습니다. 이동일자, 상품명, Kg를 확인해주세요.")
                    else:
                        st.success(f"총 {len(valid)}건의 유효한 데이터가 확인되었습니다.")

                        # 미리보기
                        preview = valid[["move_date", "meat_code", "meat_name", "origin_grade", "kg", "move_amount", "tracking_number"]].copy()
                        preview = preview.rename(columns={
                            "move_date": "이동일자",
                            "meat_code": "상품코드",
                            "meat_name": "상품명",
                            "origin_grade": "원산지(등급)",
                            "kg": "Kg",
                            "move_amount": "이동금액",
                            "tracking_number": "이력번호",
                        })
                        st.dataframe(preview, use_container_width=True, hide_index=True)

                        if st.button("💾 업로드 확정", type="primary", use_container_width=True, key="rawmeat_upload_confirm"):
                            rows = []
                            for _, r in valid.iterrows():
                                rows.append({
                                    "move_date": r["move_date"],
                                    "meat_code": r["meat_code"],
                                    "meat_name": r["meat_name"],
                                    "origin_grade": r["origin_grade"],
                                    "kg": float(r["kg"]),
                                    "move_amount": float(r["move_amount"]),
                                    "tracking_number": r["tracking_number"],
                                    "product_name": "",
                                    "production_kg": 0.0,
                                    "memo": "",
                                    "completed": False,
                                })
                            try:
                                insert_raw_meat_inputs(rows)
                                st.session_state["_rawmeat_upload_success"] = f"✅ {len(rows)}건 업로드 완료!"
                                st.rerun()
                            except Exception as e:
                                st.error(f"❌ 업로드 실패: {str(e)}")

            except Exception as e:
                st.error(f"❌ 파일 읽기 실패: {str(e)}")

    # ========================
    # 데이터 삭제
    # ========================
    elif menu == "🗑️ 데이터 삭제":
        st.header("투입 원육 데이터 삭제")

        df = load_raw_meat_inputs()

        if df.empty:
            st.info("투입된 원육이 없습니다.")
        else:
            st.warning("⚠️ 삭제된 데이터는 복구할 수 없습니다.")

            # 날짜별 삭제
            all_dates = sorted(df["move_date"].dropna().unique().tolist(), reverse=True)
            sel_dates = st.multiselect(
                "삭제할 날짜 선택",
                options=all_dates,
                placeholder="날짜를 선택하세요",
                key="rawmeat_del_dates"
            )

            if sel_dates:
                target = df[df["move_date"].isin(sel_dates)]
                st.caption(f"삭제 대상: **{len(target)}건**")

                if st.button("🗑️ 삭제", type="primary", key="rawmeat_del_btn"):
                    st.session_state["confirm_delete_rawmeat"] = True

                if st.session_state.get("confirm_delete_rawmeat"):
                    st.error(f"정말로 {len(target)}건을 삭제하시겠습니까?")
                    col_a, col_b, _ = st.columns([1, 1, 4])
                    with col_a:
                        if st.button("✅ 삭제 확인", key="confirm_del_rawmeat"):
                            for _, row in target.iterrows():
                                delete_raw_meat_input(row["id"])
                            st.success("✅ 삭제 완료!")
                            st.session_state["confirm_delete_rawmeat"] = False
                            st.rerun()
                    with col_b:
                        if st.button("❌ 취소", key="cancel_del_rawmeat"):
                            st.session_state["confirm_delete_rawmeat"] = False
                            st.rerun()
